...
class wArLeY_DBMS{
			...
			try{
				switch($this->database_type){
					...
					case "odbc":
						...
						$this->con = new PDO('odbc:Driver=FreeTDS; Server='.$this->host.'; Port='.$this->port.'; Database='.$this->database.'; UID='.$this->user.'; PWD='.$this->password.';');
						break;
					case "oracle":
	...
	* @return object $query contains result set. */
	...
	public function img($sql_statement){
		$this->err_msg = "";
		if($this->con!=null){
			try{
				$this->sql=$sql_statement;
