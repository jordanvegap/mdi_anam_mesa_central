    ...
    */
    ...
    self.init = function() {
        ...
        self.DataGridTicketsAdmin = $("#Data_Grid_Ticket_Admin").dxDataGrid({
            ...
            columns: [
                ...
                {
                    caption: "VPN",
                    dataField: "CODIGO_VPN",
                    alignment: "left",
                    width: 90,
                },
                {
                    caption: "ADUANA/VPN",
                    dataField: "UBICACION_ADUANA_ALMACEN",
                    alignment: "left",
                    width: 130,
                    // lookup: {
                ...
                {
                    caption: "USUARIO QUE AUTORIZA",
                    dataField: "NOM_EMP_SOL",
                    alignment: "left",
                    width: 250,
                },
                // {
                //     caption: "QUIEN LIBERA",
                //     dataField: "TIC_SOLICITA",
                //     alignment: "left",
                //     width: 250,
                // },
                        ...
                        if (rowData.TIC_CERRADO == 1) {
                            switch (rowData.CAM_CSC_SERVICIO_SOLICITA) {
                                case 38:
                                    ...
                                    var duracionEnSegundos = calcularDuracionEnSegundos(fechaAlta, fechaCierre, 'VencimientoSemaforo');
                            ...
                            switch (rowData.CAM_CSC_SERVICIO_SOLICITA) {
                                case 38:
                                    var duracionEnSegundos = calcularDuracionEnSegundos(fechaAlta, fechaActual, 'VencimientoSemaforo');
                        ...
                        if (rowData.TIC_CERRADO == 1) {
                            switch (rowData.CAM_CSC_SERVICIO_SOLICITA) {
                                case 38:
                                    ...
                                    let fechaCierre  =  moment(rowData.fecha_cierre_timezone).format('YYYY-MM-DD HH:mm:ss');
                                    let duracionEnSegundos = calcularDuracionEnSegundos(fechaAlta, fechaCierre, 'VencimientoSemaforo');
            ...
            onRowDblClick: function (e) {
                //setTimeout(() => {
                    AbrirPopMesaAyuda('LECTURA',e.data.CAM_MESA_CSC,e.data.TIC_NEWID) 
                //}, 1000);
            }
    ...
    self.cargaArbolTicket = function () {
        ...
        __Reques_ajax(getJSON(DeveloperType).ApiTickets_v2.url+'Get_Mesa_Empleado','GET',jsonServEmpl,getJSON(DeveloperType).ApiTickets_v2.token).then((in_emp)=>{
            if (in_emp.success == true) {
                let DataServicios = jslinq(in_emp.JsonData).where(function(el) { return el.CAM_MESA_CSC == 1; }).toList();
    ...
    self.ConsultaTickets = function(itemData){
        ...
        __Reques_ajax(getJSON(DeveloperType).ApiTickets_v2.url+'Get_Reporte_Bitacoras','GET',SearchTicket,getJSON(DeveloperType).ApiTickets_v2.token).then((all_data)=>{
            if (all_data.success == true){
    ...
    self.getPermisosEmpleadoServicios= async function(){
        ...
        let in_emp  = await  __Reques_ajax(getJSON(DeveloperType).ApiGeneral.url + 'getTreeCliProyCamp', 'GET', jsonBusqueda, getJSON(DeveloperType).ApiGeneral.token)
            if (in_emp.success == true) {
                let _T_Documento = in_emp.JsonData

                let filtro1 = jslinq(_T_Documento).where(function(el) { return el.CAM_CSC_SERVICIO == 24; }).toList();
                console.log(filtro1);
                

                filtro1.forEach(function(item) {self._ClienteProyectoCampania_ObjData.insert(item);});
                var finalData = jslinq(filtro1).select(function(el){return el.CAM_CSC_SERVICIO;}).toList();
