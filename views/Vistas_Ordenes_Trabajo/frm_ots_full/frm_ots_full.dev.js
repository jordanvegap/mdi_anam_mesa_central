SalesDashboard.dashboardModel = function() {
    ...
    var _keyProvInfra = 'CAT_PROVEEDOR_INFRA_CSC';
    var _keyCentroCostos = 'SAMT_CSC_CENTRO_COSTOS';
    ...
    self.init = function() {
        ...
        if (getUrlParam('TIPO') == 'EDITAR') {
            ...
            self.Get_OrdenTrabajos(__Oj_Dts_Busqueda,false,null);
                        ...
                        items:[
                            {
                                ...
                                editorOptions: {
                                    ...
                                    dataSource: new DevExpress.data.DataSource({
                                        store: new DevExpress.data.CustomStore({
                                            ...
                                            loadMode: "raw", paginate: false,   
                                            load: async function () {
                                                var _ary = {Tbl:"SAMT_CAT_PROVEEDORES_INFRA",NACTIVE:"CAT_PROVEEDOR_INFRA_ACTIVO"};
                                                return __Get_catalogos(getJSON(DeveloperType).ApiGeneral.url+'Get_Cat','GET',_ary,getJSON(DeveloperType).ApiGeneral.token).then((all_data)=>{
                                                    if (all_data.success == true){
                                                        return jslinq( all_data.JsonData ).orderBy(function(el){return el.CAT_PROVEEDOR_INFRA_RAZONSOCIALNOMBRE;}).toList();;
                                                    }
                                                    else {
                                                        console.log(all_data.message);
                                                    }
                                ...
                                editorOptions: {
                                    searchEnabled:true,
                                    displayExpr: "MDA_DESCRIPCION",
                                    valueExpr: "MDA_CVEMONEDA",
                                    dataSource: new DevExpress.data.DataSource({
                                        store: new DevExpress.data.CustomStore({
                                            key: _keyProvInfra,
                                            loadMode: "raw", paginate: false,   
                                            load: async function () {
                                                var _ary = {Tbl:"SAMT_MONEDAS",NACTIVE:"MDA_MONEDABASE"};
                                                return __Get_catalogos(getJSON(DeveloperType).ApiGeneral.url+'Get_Cat','GET',_ary,getJSON(DeveloperType).ApiGeneral.token).then((all_data)=>{
                                                    if (all_data.success == true){
                                                        return jslinq( all_data.JsonData ).orderBy(function(el){return el.MDA_DESCRIPCION;}).toList();;
                                                    }
                                                    else {
                                                        console.log(all_data.message);
                                                    }
                            ...
                            {
                                colSpan:2,
                                dataField: "SAMT_CSC_CENTRO_COSTOS",
                                editorType: "dxSelectBox",
                                label: {
                                ...
                                editorOptions: {
                                    searchEnabled:true,
                                    displayExpr: "SAMT_TIPO_CENTRO_IDIOMA1",
                                    valueExpr: "SAMT_CSC_CENTRO_COSTOS",
                                    dataSource: new DevExpress.data.DataSource({
                                        store: new DevExpress.data.CustomStore({
                                            key: _keyCentroCostos,

                                            loadMode: "raw", paginate: false,   
                                            load: async function () {
                                                var _ary = {Tbl:"SAMT_TIPO_CENTRO_COSTOS",NACTIVE:"SAMT_TIPO_CENTRO_ACTIVO"};
                                                return __Get_catalogos(getJSON(DeveloperType).ApiGeneral.url+'Get_Cat','GET',_ary,getJSON(DeveloperType).ApiGeneral.token).then((all_data)=>{
                                                    if (all_data.success == true){
                                                        return jslinq( all_data.JsonData ).orderBy(function(el){return el.SAMT_TIPO_CENTRO_IDIOMA1;}).toList();;
                                                    }
                                                    else {
                                                        console.log(all_data.message);
                                                    }
        ...
        if (__Frm_Orden_Trabajo_servicio.validate().isValid === true) {
            ...
            __Reques_ajax(getJSON(DeveloperType).ApiOrdenTrabajo_v2.url+'Insert_Orden_Trabajo','POST',JSON.stringify(objInsert),getJSON(DeveloperType).ApiOrdenTrabajo_v2.token).then((in_emp)=>{
                if (in_emp.success == true) {
                    ...
                    self.Get_OrdenTrabajos(__Oj_Dts_Busqueda,true,null);
                }
                else {
                    loadPanel.hide();
                }
    ...
    self.Get_OrdenTrabajos=function(__AryDatos,PopAlert,DgRefresh){
        loadPanel.show();
        var jsonBusqueda = {
            EMP_CLV_EMPRESA:localStorage.getItem('EMP_CLV_EMPRESA'),
            EMP_CSC_EMPRESA_HOST:localStorage.getItem('EMP_CSC_EMPRESA_HOST'),
               ...
               /* var JSonData = result.JsonData;
                ...
                for (let i = 0; i < JSonData.length; i++) {
                    ...
                    var imgage=document.createElement("img");
                        imgage.setAttribute("src", JSonData[i].FOTO_IMAGEN_URL);
                        imgage.setAttribute("data-src", JSonData[i].FOTO_IMAGEN_URL);
                        imgage.className = "preload-image shadow-medium";
                        Ahref.appendChild(imgage);
                }*/
            } else {
                console.log(result);
                loadPanel.hide();
