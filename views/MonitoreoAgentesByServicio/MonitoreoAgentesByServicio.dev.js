SalesDashboard.dashboardModel = function() {
    ...
    socketRealTimeAgentes.on(localStorage.getItem('EMP_CLV_EMPRESA')+"_AGENTES", function(data) {
        var Campa = jslinq(data.JsonData).where(function(el){return el.CAM_CSC_SERVICIO == CampaMonitorea;}).toList();
    ...
    socketRealTimeAgentes.on(localStorage.getItem('EMP_CLV_EMPRESA')+"_CDR", function(data) {
        ...
        var result = jslinq(dts).groupBy(function(el){return el.CAM_CSC_SERVICIO;}).toList();
        var Campa = jslinq(result).where(function(el){return el.key == CampaMonitorea;}).toList();
